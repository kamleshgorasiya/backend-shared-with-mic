...
const errorsVisitor: Visitor<any> = {
    ...
    InterfaceTypeDefinition(node: InterfaceTypeDefinitionNode) {
        ...
        return [
            ...
            ...node.fields
                .filter(f => !(f as any).includes('errorCode:'))
                .map(f => `${f};`),
            `}`,
        ].join('\n')
    },
...
function generateTypeResolvers(schema: GraphQLSchema) {
    ...
    const queries = getOperationsThatReturnErrorUnions(
        schema,
    ...
    for (const operation of operations) {
        ...
        if (!typesHandled.has(returnType.name)) {
            ...
            result.push(
                ...
                `      return isGraphQLError(value) ? (value as any).__typename : '${
                    nonErrorResult!.name
                }';`,
                `    },`,
                `  },`
...
function getOperationsThatReturnErrorUnions(
...
function inheritsFromErrorResult(
