...
let InitializerService = class InitializerService {
    constructor(connection, zoneService, channelService, roleService, administratorService, shippingMethodService, globalSettingsService) {
    ...
    async awaitDbSchemaGeneration() {
        ...
        for (let attempt = 0; attempt < retries; attempt++) {
            try {
                const result = await this.connection.getRepository(administrator_entity_1.Administrator).find();
                return;
            }
            catch (e) {
