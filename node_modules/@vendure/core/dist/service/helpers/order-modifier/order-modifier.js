...
let OrderModifier = class OrderModifier {
    constructor(connection, configService, orderCalculator, paymentService, countryService, stockMovementService, productVariantService, customFieldRelationService, promotionService, eventBus, entityHydrator, historyService) {
    ...
    async getAdjustmentFromNewlyAppliedPromotions(ctx, order) {
        ...
        const newPromotionDiscounts = order.discounts
            ...
            return !existingPromotions.find(p => utils_1.idsAreEqual(p.id, promotionId));
        })
            .filter(discount => {
