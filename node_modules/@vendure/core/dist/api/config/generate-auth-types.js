...
function generateAuthenticationTypes(schema, authenticationStrategies) {
    ...
    for (const strategy of authenticationStrategies) {
        const inputSchema = graphql_1.buildASTSchema(strategy.defineInputType());
        const inputType = Object.values(inputSchema.getTypeMap()).find((type) => graphql_1.isInputObjectType(type));
        if (!inputType) {
