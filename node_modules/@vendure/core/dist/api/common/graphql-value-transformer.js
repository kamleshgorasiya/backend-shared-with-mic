...
class GraphqlValueTransformer {
    ...
    getOutputTypeTree(document) {
        ...
        const visitor = {
            enter: node => {
                ...
                if (node.kind === 'Field') {
                    const newNode = {
                        type: (type && graphql_1.getNamedType(type)) || undefined,
        ...
        for (const operation of document.definitions) {
            graphql_1.visit(operation, graphql_1.visitWithTypeInfo(typeInfo, visitor));
        }
    ...
    getInputTypeTree(definition) {
        ...
        const visitor = {
            enter: node => {
                if (node.kind === 'Argument') {
                    ...
                    if (args) {
                        const inputType = graphql_1.getNamedType(args.type);
                        const newNode = {
                        ...
                        if (graphql_1.isInputObjectType(inputType)) {
                            if (graphql_1.isInputObjectType(inputType)) {
        ...
        graphql_1.visit(definition, graphql_1.visitWithTypeInfo(typeInfo, visitor));
    ...
    isList(t) {
        return graphql_1.isListType(t) || (graphql_1.isNonNullType(t) && graphql_1.isListType(t.ofType));
    }
