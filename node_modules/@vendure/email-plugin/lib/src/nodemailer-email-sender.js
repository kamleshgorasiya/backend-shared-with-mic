...
class NodemailerEmailSender {
    async send(email, options) {
        switch (options.type) {
            case 'none':
                ...
                break;
            case 'file':
                const fileName = normalize_string_1.normalizeString(`${new Date().toISOString()} ${email.recipient} ${email.subject}`, '_');
            ...
            case 'testing':
                ...
                break;
            default:
                return shared_utils_1.assertNever(options);
        }
    }
    getSmtpTransport(options) {
        if (!this._smtpTransport) {
            options.logger = options.logging ? this.createLogger() : false;
            this._smtpTransport = nodemailer_1.createTransport(options);
        }
        return this._smtpTransport;
    }
    getSendMailTransport(options) {
        if (!this._sendMailTransport) {
            this._sendMailTransport = nodemailer_1.createTransport(Object.assign({ sendmail: true }, options));
        }
    ...
    createLogger() {
        function formatError(args) {
            ...
            return util_1.format(message, ...params);
        }
        return {
            level(level) {
                /* noop */
