...
class CompatibilityPluginVisitor extends visitorPluginCommon.BaseVisitor {
    constructor(rawConfig, _schema, options) {
        super(rawConfig, {
            ...
            scalars: visitorPluginCommon.buildScalarsFromConfig(_schema, rawConfig),
        });
    ...
    buildOperationBlock(node) {
        ...
        const baseName = this.convertName(node.name.value, { suffix: `${changeCaseAll.pascalCase(node.operation)}` });
        ...
        const selectionSetTypes = {
            [typesPrefix + this.convertName('Variables')]: {
                export: 'type',
                name: this.convertName(node.name.value, { suffix: `${changeCaseAll.pascalCase(node.operation)}Variables` }),
            },
        };
        ...
        if (this.config.reactApollo) {
            ...
            if (hooks) {
                selectionSetTypes['use' + prefix] = {
                    export: 'const',
                    name: 'use' + this.convertName(baseName, { suffix: changeCaseAll.pascalCase(node.operation) }),
                };
            }
        }
