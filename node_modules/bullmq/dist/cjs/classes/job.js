...
class Job {
    ...
    opts = {}, id) {
        ...
        this.parentKey = utils_1.getParentKey(opts.parent);
    ...
    async moveToFailed(err, token, fetchNext = false) {
        ...
        if (moveToFailed) {
            const args = scripts_1.Scripts.moveToFailedArgs(queue, this, message, this.opts.removeOnFail, token, fetchNext, this.opts.attempts && this.attemptsMade >= this.opts.attempts
                ? this.attemptsMade
                : 0);
        ...
        if (code < 0) {
            throw scripts_1.Scripts.finishedErrors(code, this.id, command, 'active');
        }
    }
    /**
     ...
     */
    changeDelay(delay) {
        return scripts_1.Scripts.changeDelay(this.queue, this.id, delay);
    }
    /**
    ...
    async waitUntilFinished(queueEvents, ttl) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            const [status, result] = (await scripts_1.Scripts.isFinished(this.queue, jobId, true));
    ...
    moveToWaitingChildren(token, opts = {}) {
        return scripts_1.Scripts.moveToWaitingChildren(this.queue, this.id, token, opts);
    }
    /**
    ...
    async promote() {
        const queue = this.queue;
        const jobId = this.id;
        const code = await scripts_1.Scripts.promote(queue, jobId);
        if (code < 0) {
            throw scripts_1.Scripts.finishedErrors(code, this.id, 'promote', 'delayed');
        }
    }
    /**
    ...
    async retry(state = 'failed') {
        ...
        this.returnvalue = null;
        return scripts_1.Scripts.reprocessJob(this.queue, this, state);
    }
    /**
