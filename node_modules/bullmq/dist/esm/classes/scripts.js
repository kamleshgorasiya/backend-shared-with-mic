...
export class Scripts {
    ...
    static async updateProgress(queue, job, progress) {
        ...
        const result = await client.updateProgress(keys, [
            job.id,
            progressJson,
        ]);
        if (result < 0) {
    ...
    static moveToFinishedArgs(queue, job, val, propVal, shouldRemove, target, token, fetchNext = true) {
        ...
        const keepJobs = pack(typeof shouldRemove === 'object'
            ? shouldRemove
    ...
    static moveToFailedArgs(queue, job, failedReason, removeOnFailed, token, fetchNext = false, retriesExhausted = 0) {
