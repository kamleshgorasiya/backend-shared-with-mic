...
export class Job {
    ...
    toJSON() {
        const _a = this, { queue } = _a, withoutQueue = __rest(_a, ["queue"]);
        return withoutQueue;
    }
    /**
    ...
    async moveToCompleted(returnValue, token, fetchNext = true) {
        ...
        return Scripts.moveToCompleted(this.queue, this, stringifiedReturnValue, this.opts.removeOnComplete, token, fetchNext);
    }
    /**
    ...
    async moveToFailed(err, token, fetchNext = false) {
        ...
        if (this.attemptsMade < this.opts.attempts && !this.discarded) {
            ...
            if (delay === -1) {
                ...
                const args = Scripts.moveToDelayedArgs(queue, this.id, Date.now() + delay);
                multi.moveToDelayed(args);
                command = 'delayed';
            }
            else {
                // Retry immediately
                multi.retryJob(Scripts.retryJobArgs(queue, this));
                command = 'retry';
            }
        }
        else {
        ...
        if (moveToFailed) {
            const args = Scripts.moveToFailedArgs(queue, this, message, this.opts.removeOnFail, token, fetchNext, this.opts.attempts && this.attemptsMade >= this.opts.attempts
                ? this.attemptsMade
                : 0);
    ...
    async waitUntilFinished(queueEvents, ttl) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            const [status, result] = (await Scripts.isFinished(this.queue, jobId, true));
    ...
    addJob(client, parentOpts) {
        ...
        return Scripts.addJob(client, queue, jobData, this.opts, this.id, parentOpts);
    }
